# ============================================================================
#  system_bd.tcl  --  AUTHORITATIVE block design for this project
# ----------------------------------------------------------------------------
#  PROVENANCE
#    Derived from the official MicroPhase ANTSDR E310 V1 design:
#      repo   https://github.com/MicroPhase/hdl
#      branch antsdr_fmcomms_no_os
#      commit a329fa58bb62bd94cc37f3e98528a1e0f6e1afe7
#      file   projects/antsdre310/system_bd.tcl
#    The pristine original is kept, unmodified and read-only, at
#      Vendor/MicroPhase_E310_V1/hdl/projects/antsdre310/system_bd.tcl
#    Diff the two files to see every change (requirements 34, 35).
#
#  CHANGES FROM THE OFFICIAL BASELINE
#    Every change is bracketed by "GNSS-CRPA MOD" comments.  There are two:
#      MOD-1  The four axi_ad9361_dac_fifo/dout_data_* -> axi_ad9361/dac_data_*
#             connections are removed, so the DAC data pins are free to be
#             driven by the custom processing block instead.
#      MOD-2  A gnss_passthrough instance is added, taking RX I/Q from
#             axi_ad9361 and the vendor DMA stream from axi_ad9361_dac_fifo,
#             and driving axi_ad9361's DAC data pins through a software
#             selectable mux.  An AXI4-Lite slave is mapped at 0x43C0_0000.
#
#    Nothing else is altered.  The PS7 configuration, DDR parameters, clocks,
#    MIO assignment, SPI, AD9361 interface, both DMA paths, interrupts and
#    reset logic are exactly as shipped by MicroPhase (requirement 4).
#
#  WHY THIS INSERTION POINT
#    axi_ad9361's ADC outputs and DAC inputs are both in the axi_ad9361/l_clk
#    domain, so the processing block sits in a single clock domain and needs no
#    resampling.  The vendor capture path (util_wfifo -> util_cpack2 ->
#    axi_dmac) is untouched and still records the same RX samples the
#    processing block sees, which makes it a true diagnostic tap
#    (requirements 47, 48).
# ============================================================================
...
# ---------------------------------------------------------------------------
# GNSS-CRPA MOD-0 BEGIN
#   adi_project_create sets ip_repo_paths to the vendor library only, and it
#   does that before it sources this file.  Append the project's own IP
#   repository here so that ad_ip_instance can resolve gnss_passthrough.
#   The path is supplied by the build scripts; it is never guessed
#   (requirements 31, 32).
# ---------------------------------------------------------------------------
if {![info exists ::env(GNSS_CRPA_IP_REPO)]} {
  error "ERROR: environment variable GNSS_CRPA_IP_REPO is not set.\
         It must point at the directory holding the packaged custom IP\
         (normally <code_r1>/Build/ip_repo).  Run the build through\
         Automation/PowerShell/Build_All.ps1, or set it manually before\
         sourcing this script."
}
set gnss_ip_repo [file normalize $::env(GNSS_CRPA_IP_REPO)]
if {![file exists [file join $gnss_ip_repo gnss_passthrough component.xml]]} {
  error "ERROR: custom IP not found at $gnss_ip_repo/gnss_passthrough/component.xml.\
         Package it first with Source/IP/gnss_passthrough/gnss_passthrough_ip.tcl."
}
...
set gnss_ipdef [get_ipdefs -all -quiet -filter \
  {VLNV =~ *:gnss_passthrough:* && design_tool_contexts =~ *IPI* && UPGRADE_VERSIONS == ""}]
if {[llength $gnss_ipdef] == 0} {
  error "ERROR: gnss_passthrough was not found in the IP catalog after registering\
         $gnss_ip_repo. Re-package it with\
         Source/IP/gnss_passthrough/gnss_passthrough_ip.tcl, or run\
         Build_All.ps1 -FromStage 3."
}
...
# ---------------------------------------------------------------------------
# GNSS-CRPA MOD-1 BEGIN
#   Upstream connected the DAC FIFO data pins straight to the AD9361 core:
...
#   Those four lines are intentionally absent here.  The data pins are driven
#   by gnss_passthrough in MOD-2 instead, which can select either this vendor
#   DMA stream or the live RX passthrough stream.  The enable/valid strobes
#   above are untouched, so the vendor DAC timing is unchanged.
# GNSS-CRPA MOD-1 END
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# GNSS-CRPA MOD-2 BEGIN
#   Custom processing block: AD9361 RX -> gnss_passthrough -> AD9361 TX.
#   Entirely in the PL, in the axi_ad9361/l_clk domain (requirements 41-46).
#
#   The ADC pins below are already driven onto nets by the adc-path wfifo
#   section above.  ad_connect resolves an already-connected pin to its net and
#   fans out, so these calls tap the existing signals rather than replacing
#   them -- the vendor DMA capture path keeps working unchanged.
# ---------------------------------------------------------------------------

ad_ip_instance gnss_passthrough gnss_passthrough

# sample-domain clock and reset (same domain as the AD9361 core interface)
ad_connect axi_ad9361/l_clk gnss_passthrough/clk
ad_connect axi_ad9361/rst   gnss_passthrough/rst

# RX taps from the AD9361 core
ad_connect axi_ad9361/adc_enable_i0 gnss_passthrough/adc_enable_i0
ad_connect axi_ad9361/adc_valid_i0  gnss_passthrough/adc_valid_i0
ad_connect axi_ad9361/adc_data_i0   gnss_passthrough/adc_data_i0
ad_connect axi_ad9361/adc_enable_q0 gnss_passthrough/adc_enable_q0
ad_connect axi_ad9361/adc_valid_q0  gnss_passthrough/adc_valid_q0
ad_connect axi_ad9361/adc_data_q0   gnss_passthrough/adc_data_q0
ad_connect axi_ad9361/adc_enable_i1 gnss_passthrough/adc_enable_i1
ad_connect axi_ad9361/adc_valid_i1  gnss_passthrough/adc_valid_i1
ad_connect axi_ad9361/adc_data_i1   gnss_passthrough/adc_data_i1
ad_connect axi_ad9361/adc_enable_q1 gnss_passthrough/adc_enable_q1
ad_connect axi_ad9361/adc_valid_q1  gnss_passthrough/adc_valid_q1
ad_connect axi_ad9361/adc_data_q1   gnss_passthrough/adc_data_q1

# TX request strobes from the AD9361 core (already driven to the rfifo)
ad_connect axi_ad9361/dac_enable_i0 gnss_passthrough/dac_enable_i0
ad_connect axi_ad9361/dac_valid_i0  gnss_passthrough/dac_valid_i0
ad_connect axi_ad9361/dac_enable_q0 gnss_passthrough/dac_enable_q0
ad_connect axi_ad9361/dac_valid_q0  gnss_passthrough/dac_valid_q0
ad_connect axi_ad9361/dac_enable_i1 gnss_passthrough/dac_enable_i1
ad_connect axi_ad9361/dac_valid_i1  gnss_passthrough/dac_valid_i1
ad_connect axi_ad9361/dac_enable_q1 gnss_passthrough/dac_enable_q1
ad_connect axi_ad9361/dac_valid_q1  gnss_passthrough/dac_valid_q1

# vendor DMA-sourced TX data (the path MOD-1 detached from the core)
ad_connect axi_ad9361_dac_fifo/dout_data_0 gnss_passthrough/dma_dac_data_i0
ad_connect axi_ad9361_dac_fifo/dout_data_1 gnss_passthrough/dma_dac_data_q0
ad_connect axi_ad9361_dac_fifo/dout_data_2 gnss_passthrough/dma_dac_data_i1
ad_connect axi_ad9361_dac_fifo/dout_data_3 gnss_passthrough/dma_dac_data_q1

# muxed TX data into the AD9361 core
ad_connect gnss_passthrough/dac_data_i0 axi_ad9361/dac_data_i0
...
# GNSS-CRPA MOD-2 (continued): control/status aperture for the custom block.
# 0x43C0_0000 is the conventional free Zynq-7000 GP0 slot and is not used by
# any peripheral in the official E310 V1 design.  Recorded in
# Source/Config/board_e310_v1.json so software and scripts agree.
ad_cpu_interconnect 0x43C00000 gnss_passthrough
